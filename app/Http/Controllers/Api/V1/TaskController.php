...
//                    DB::rollBack();
    ...
    public function update(Request $request , $id)
    {
        ...
        //检查是否是任务成员，不是不能更改
        $taskUser = TaskUser::where('task_id' , $id)->where('selected_user_id' , $user_id)->orWhere('user_id' , $user_id)->first();
        if(!$taskUser){
    ...
    public function stage(Request $request)
    {
        ...
        try {
            ...
            if($tier == 0){
                ...
                //检查是否是任务成员，不是不能更改
                $taskUser = TaskUser::where('task_id' , $task->pid)->where('selected_user_id' , $this->auth_user_id)->orWhere('user_id' , $this->auth_user_id)->first();
                if(!$taskUser){
                    return $this->response->array($this->apiError('没有权限更改任务', 403));
                }
